...
CompMgm  <- R6::R6Class ( 
  ...
  private = list(
    ...
    .name_param = function(p){
      r <- paste(p$values$id_comp, p$values$id_value_set, sep = ':')
      if (!is.null(p$values[["trial"]])){
        r <- paste(r, p$values$trial, sep = ':')
      }
      return(r)
    },
    ...
    .number_to_task = function(id_or_number){
      if (is.numeric(id_or_number)){
        return(private$.obj_log$get_all_tasks()[id_or_number])
      } else if (is.character(id_or_number)){
        return(intersect(private$.obj_log$get_all_tasks(), id_or_number))
      }
      stop('Wrong data type. Should be character or integer.')
    }
  ),
  active = list(
    params = function() {
  ...
  public = list(
    ...
    get_task = function(id){
      private$.obj_log$get_tasks(id)[[1]]
    },
    ...
    get_all_unfinished_task_names = function(){
      n <- private$.obj_log$get_all_unfinished_tasks()
      names(private$.obj_log$get_tasks(n))
    },
    get_all_finished_task_names = function(){
      n <- private$.obj_log$get_all_finished_tasks()
      names(private$.obj_log$get_tasks(n))
    },
    get_all_task_names = function(){
      names(private$.obj_log$get_tasks())
    },
