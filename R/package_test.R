...
x <- snowfall::sfSapply(1:2, 
   fun = function(x, a){ 
      ...
      batch$create_task(id = paste0('TTT2', x, collapse  = '.'), name = 'task2', descr = 'dos', notes = "not sure what's going on")
      batch$create_task(id = paste0('XXX3', x, collapse  = '.'), name = 'task3', descr = 'dos', notes = "not sure what's going on", depends = c('t1', 't2'))
      batch$write()
   },
   a = batch$filename 
)
